...
import { getUserSpaces } from 'apiSdk/user-spaces';
import { UserSpaceInterface } from 'interfaces/user-space';

import { SpaceListPage as ShowCaseList } from 'pages/spaces';

function DashboardPage() {
  const { session } = useSession();

  const spaceStat = useSWR<PaginatedInterface<SpaceInterface>>(
    () => `/spaces`,
    () => getSpaces({ limit: 1, offset: 0 }),
  );
  ...
  const userSpaceStat = useSWR<PaginatedInterface<UserSpaceInterface>>(
    () => `/user-spaces`,
    () => getUserSpaces({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Spaces',
      result: spaceStat?.data?.totalCount,
      isLoading: spaceStat.isLoading,
    ...
    {
      title: 'User Spaces',
      result: userSpaceStat?.data?.totalCount,
      isLoading: userSpaceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`spaces`} passHref legacyBehavior>
                  <Button
