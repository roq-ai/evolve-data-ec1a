...
import { getUserSpaceById, updateUserSpaceById } from 'apiSdk/user-spaces';
...
function UserSpaceEditPage() {
  ...
  const handleSubmit = async (values: UserSpaceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateUserSpaceById(id, values);
      mutate(updated);
      resetForm();
