...
export function SpaceListPage(props: SpaceListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getSpaces({
        relations: ['business', 'data_source.count', 'user_space.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('business', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('data_source', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'data_source',
          header: 'Data Source',
          accessorKey: 'data_source',
          cell: ({ row: { original: record } }: any) => record?._count?.data_source,
        }
      : null,
    hasAccess('user_space', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_space',
          header: 'User Space',
          accessorKey: 'user_space',
          cell: ({ row: { original: record } }: any) => record?._count?.user_space,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/spaces/view/${record.id}`} passHref legacyBehavior>
                <Button
